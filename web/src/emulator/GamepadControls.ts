 ...
 * title/credits screens and beginning actual gameplay — POP treats any
 * ordinary keypress as "continue," so this dispatches a synthetic Enter on
 * the canvas the same way TouchControls.ts dispatches a synthetic Ctrl+J,
 * rather than needing the game to know anything about gamepads at all.
 *
 ...
 */
export function attachGamepadControls(
    io: Apple2IO,
    canvas: HTMLCanvasElement,
    rewindButton: { rewind: () => void }
): GamepadControlsHandle {
    let engaged = false;
    let connectedIndex: number | null = null;
    let actionWasPressed = false;
    let rewindWasPressed = false;
    ...
    function update(): void {
        ...
        const rewindPressed = pad.buttons[BUTTON_REWIND]?.pressed ?? false;
        if (rewindPressed && !rewindWasPressed) {
            rewindButton.rewind();
        }
        rewindWasPressed = rewindPressed;
